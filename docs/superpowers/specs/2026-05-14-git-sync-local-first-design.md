...
首次绑定前，本地仓库会切换或创建到用户选定的目标分支，确保本地分支名与同步目标一致。
...
冲突态下的主要恢复入口是“手动处理后继续同步”。该入口会保留本地分支和工作区状态，不会自动回滚到远端版本；用户完成手动处理后再次触发同步即可。

### 6. 重构边界

本次设计不需要考虑旧兼容或历史迁移路径。应用尚未发布使用，因此可以直接按新模型重构同步子系统、状态存储和设置页交互，而不必保留旧的 GitHub OAuth 状态、旧绑定文件格式或旧 UI 入口。
...
7. Main 进程切换或创建本地目标分支
8. 如果远端为空，执行首次提交并 push
9. 如果远端已有内容，确认目标分支可用后执行后续同步策略
