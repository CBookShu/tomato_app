...
2. 创建保存分支
   git branch "local-backup-{timestamp}"
...
1. 显示操作指引面板
2. 用户在终端操作：
   a. cd {tomato-data-path}
   b. git merge local-backup-{timestamp}
   c. 编辑冲突文件，保留正确内容
   d. git add . && git commit -m "resolve conflict"
   e. git push origin main
3. 用户在应用中点击"重新同步"
4. 应用检测同步状态：
   - 检查 git status 是否干净
   - 检查本地是否与远程同步
   - 同步成功 → 删除 local-backup 分支 → 恢复正常状态
```
...
1. 保持当前本地状态（已在 local-backup 分支保存）
2. 切回 main 分支
   git checkout main
3. 重置到远程状态
   git fetch origin
   git reset --hard origin/main
4. 提示用户"已回退到远程版本，本地变更已保存在分支 local-backup-{timestamp}"
5. 恢复正常状态
```
...
正常状态 ──同步──→ 检查结果
              ...
              自行处理冲突                    放弃同步，回退
                    │                               │
                    ▼                               ▼
              用户 merge 解决               reset --hard origin/main
                    │                               │
                    ▼                               ▼
              重新同步 → 成功                正常状态
                    │
...
│                                     │
│ 本地数据已保存到分支：                │
│ local-backup-2026-05-10-143022      │
│                                     │
│ 请选择处理方式：                      │
│                                     │
│ [自行处理冲突]    [放弃同步，回退]     │
...
| 冲突 | 检测到冲突，等待选择 | 自行处理冲突 / 放弃同步 |
| 处理中 | 用户选择自行处理，等待解决 | 打开终端、复制路径、重新同步 |
